...
public class AdminQuestAnswerController {
	...
	//파일 다운로드
			@RequestMapping("/adminQna/adminQuestAnswerFile.do")
			public ModelAndView download(@RequestParam("num") int num) {
				...
				mav.addObject("downloadFile", questAnswerVO.getQ_uploadfile());
				mav.addObject("filename", questAnswerVO.getQ_filename());
				
				return mav;		
			}
			
		//이미지 처리
			@RequestMapping("/adminQna/adminQuestAnswerImageView.do")
			public ModelAndView viewImage(@RequestParam("num") int num) {
				...
				mav.addObject("imageFile", questAnswerVO.getQ_uploadfile());
				mav.addObject("filename", questAnswerVO.getQ_filename());
